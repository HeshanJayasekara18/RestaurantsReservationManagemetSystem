...
export function HeroSection() {
  ...
  return (
    <section className="relative flex min-h-screen items-center overflow-hidden bg-background">
      ...
      <div
        className="absolute top-0 right-0 hidden dark:block h-[600px] w-[600px] rounded-full bg-amber-500/5 blur-[120px]"
        aria-hidden="true"
      />
      <div
        className="absolute bottom-0 left-0 hidden dark:block h-[500px] w-[500px] rounded-full bg-zinc-800/10 blur-[100px]"
        aria-hidden="true"
      />
      ...
      <div className="relative mx-auto grid w-full max-w-7xl gap-12 px-6 py-32 lg:grid-cols-2 lg:items-center lg:gap-16 lg:px-8 lg:py-0">
        ...
        <div className="flex flex-col gap-8">
          ...
          {/* Headline */}
          <h1
            className={`font-serif text-5xl leading-[1.1] tracking-tight text-foreground transition-all duration-700 delay-100 md:text-6xl lg:text-7xl ${
              mounted
                ? "translate-y-0 opacity-100"
                : "translate-y-6 opacity-0"
            }`}
          ...
          {/* Subtitle */}
          <p
            className={`max-w-lg text-lg leading-relaxed text-muted-foreground transition-all duration-700 delay-200 ${
              mounted
                ? "translate-y-0 opacity-100"
                : "translate-y-6 opacity-0"
            }`}
          ...
          >
            ...
            <div className="flex -space-x-2">
              {[
                ...
                <div
                  key={i}
                  className={`flex h-8 w-8 items-center justify-center rounded-full border-2 border-background text-[10px] font-medium text-white ${bg}`}
                >
                  {["JD", "AL", "MK", "RS"][i]}
                </div>
              ))}
